...
struct EnvironmentInfo {
	...
	EnvironmentInfo() {}
	EnvironmentInfo(const Environment& env) {
		root = env.root;
		name = env.name;
		active_scene = env.active_scene;
	...
	void serialize(Ar& ar) {
		last_open_time = std::chrono::system_clock::now();
		ar & YAS_OBJECT(nullptr,root,name,active_scene,last_open_time);
	}
};


struct RecentEnvrionment {
	std::map<std::string,EnvironmentInfo>		recent_env;
	...
	void AddEnvironement(const Environment& env) {
		recent_env[env.name] = EnvironmentInfo(env);
	}

	template<typename Ar>
	void serialize(Ar& ar) {
		ar & YAS_OBJECT(nullptr,recent_env);
	}
};
#endif